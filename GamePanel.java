...
public class GamePanel extends JPanel implements MouseMotionListener {
		...
		addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				...
				if 		(e.getKeyCode() == KeyEvent.VK_LEFT)
					...
					level.player1.setVelocity(Player.VELOCITY, 0);
				else if (e.getKeyCode() == KeyEvent.VK_DOWN && (playerOneIsOnBar || playerOneIsOnLadder))
					level.player1.setVelocity(0, Player.VELOCITY);
				else if (e.getKeyCode() == KeyEvent.VK_UP && playerOneIsOnLadder)
					level.player1.setVelocity(0, -Player.VELOCITY);
	...
	public static String getDeathMessage() {
		String[] options = {
				...
				"Essayez encore.",
				"Sa fait male.",
				"Bien, c'est embarassant. Tu est mort. Essayez Encore!",
				"Tu est Mort, mais au moin il ya une maison pour vous amélioré !.",
		};
		return options[(int) (Math.random() * options.length)];
	}
	/**
	...
	public String getLoseMessage() {
		String[] options = {
				"tu a perdu !",
				"Game over.",
				"Tous tes Or nous appartient !",
				"Game over! pas prêt pour la grand league !!",
				"Game over! vous pouvez faire plus de pratique !!",
		};
		return options[(int) (Math.random() * options.length)];
	}
